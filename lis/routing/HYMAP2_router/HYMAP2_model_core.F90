...
subroutine HYMAP2_model_core(n,it,mis,nseqall,nz,time,dt,  &
     ...
     flowtype,drvel,drtotwth,drrad,drman,drslp,drtotlgh,drnoutlet,drstomax,drout,drsto,inletarea)
  ! ================================================
  !HYMAP2 core 
  !Augusto Getirana 
  !6th Dec 2010
...
!  real, allocatable    :: next_glb(:)
  ...
  real,    intent(inout)  :: drvel(nseqall)  
  real,    intent(in)  :: drtotwth(nseqall) 
  real,    intent(inout)  :: drrad(nseqall)     
  real,    intent(inout)  :: drman(nseqall)    
  real,    intent(inout)  :: drslp(nseqall)    
  real,    intent(inout)  :: drtotlgh(nseqall)   
  real,    intent(inout)  :: drnoutlet(nseqall)   
  real,    intent(inout)  :: inletarea(nseqall)    !urban drainage outlet id [-]: 0 - network; 1 outlet
  real,    intent(inout)  :: drstomax(nseqall)    !maximum urban drainage water storage capacity [m3]
...
!      ic_down=next(ic)
    ...
    elseif(flowmap(ic)==4)then
      ...
      call HYMAP2_calc_urb_drain_out(dt,drrad(ic),drman(ic),drslp(ic),drstomax(ic),&
  ...
  if(flowtype==4)then
    ...
    do ic=1,nseqall
      if(flowmap(ic)==4)then
        ...
        call HYMAP2_calc_urban_drain_stonxt(dt,drstomax(ic),drout(ic),drinf(ic),rivdph(ic),drsto(ic),rivsto(ic),inletarea(ic),drvel(ic))
      endif
    enddo
