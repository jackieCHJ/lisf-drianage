...
! === urban drainage and flood modeling ===
  ...
  real,   allocatable         :: drvel(:)       ! gutter water intake velocity [m/s]
  real,   allocatable  :: inletarea(:)     !sum of gutter width within HyMAP grid cell [m]
  !drainage network parameters
  real,   allocatable                        :: drblk(:)     ! street block length [m]
  real,   allocatable                        :: drrad(:)     ! underground squared pipe width [m]
  real,   allocatable                        :: drlgh(:)     ! drainage length density [m/m2]
  real,   allocatable                        :: drman(:)   ! Roughness coefficient for ciment pipes [-]
  real,   allocatable                        :: drslp(:)      ! drainage system slope [m/m]
   ...
   !ag(27Apr2020)
    ...
    do n=1, LIS_rc%nnest
       ...
       if(HYMAP2_routing_struc(n)%flowtype ==4)then
          ...
          !pipe 初始化
          allocate(HYMAP2_routing_struc(n)%drvel(HYMAP2_routing_struc(n)%nseqall))
          allocate(HYMAP2_routing_struc(n)%inletarea(HYMAP2_routing_struc(n)%nseqall))
          allocate(HYMAP2_routing_struc(n)%drblk(HYMAP2_routing_struc(n)%nseqall))
          allocate(HYMAP2_routing_struc(n)%drrad(HYMAP2_routing_struc(n)%nseqall))
          allocate(HYMAP2_routing_struc(n)%drlgh(HYMAP2_routing_struc(n)%nseqall))
          allocate(HYMAP2_routing_struc(n)%drman(HYMAP2_routing_struc(n)%nseqall))
          allocate(HYMAP2_routing_struc(n)%drslp(HYMAP2_routing_struc(n)%nseqall))          
...
!    do n=1, LIS_rc%nnest
       if(HYMAP2_routing_struc(n)%flowtype==4)then
          ...
          call HYMAP2_grid2vector(LIS_rc%lnc(n),LIS_rc%lnr(n),&
               ...
               HYMAP2_routing_struc(n)%seqx,&
               HYMAP2_routing_struc(n)%seqy,&
               tmp_real,HYMAP2_routing_struc(n)%drrad)
       else
          HYMAP2_routing_struc(n)%drrad=HYMAP2_routing_struc(n)%imis
       endif  
    ctitle = 'HYMAP_urban_drlgh'
!    do n=1, LIS_rc%nnest
       if(HYMAP2_routing_struc(n)%flowtype==4)then
          call HYMAP2_read_param_real_2d(ctitle,n,tmp_real)
          call HYMAP2_grid2vector(LIS_rc%lnc(n),LIS_rc%lnr(n),&
               ...
               HYMAP2_routing_struc(n)%seqx,&
               HYMAP2_routing_struc(n)%seqy,&
               tmp_real,HYMAP2_routing_struc(n)%drlgh)
       else
          HYMAP2_routing_struc(n)%drlgh=HYMAP2_routing_struc(n)%imis
       endif   
    ctitle = 'HYMAP_urban_inletarea'
!    do n=1, LIS_rc%nnest
       if(HYMAP2_routing_struc(n)%flowtype==4)then
          call HYMAP2_read_param_real_2d(ctitle,n,tmp_real)
          call HYMAP2_grid2vector(LIS_rc%lnc(n),LIS_rc%lnr(n),&
               ...
               HYMAP2_routing_struc(n)%seqx,&
               HYMAP2_routing_struc(n)%seqy,&
               tmp_real,HYMAP2_routing_struc(n)%inletarea)
       else
          HYMAP2_routing_struc(n)%inletarea=HYMAP2_routing_struc(n)%imis
       endif
    ctitle = 'HYMAP_urban_drman'
!    do n=1, LIS_rc%nnest
       if(HYMAP2_routing_struc(n)%flowtype==4)then
          call HYMAP2_read_param_real_2d(ctitle,n,tmp_real)
          call HYMAP2_grid2vector(LIS_rc%lnc(n),LIS_rc%lnr(n),&
               ...
               HYMAP2_routing_struc(n)%seqx,&
               HYMAP2_routing_struc(n)%seqy,&
               tmp_real,HYMAP2_routing_struc(n)%drman)
       else
          HYMAP2_routing_struc(n)%drman=HYMAP2_routing_struc(n)%imis
       endif   
    ctitle = 'HYMAP_urban_drslp'
!    do n=1, LIS_rc%nnest
       if(HYMAP2_routing_struc(n)%flowtype==4)then
          call HYMAP2_read_param_real_2d(ctitle,n,tmp_real)
          call HYMAP2_grid2vector(LIS_rc%lnc(n),LIS_rc%lnr(n),&
               ...
               HYMAP2_routing_struc(n)%seqx,&
               HYMAP2_routing_struc(n)%seqy,&
               tmp_real,HYMAP2_routing_struc(n)%drslp)
       else
          HYMAP2_routing_struc(n)%drslp=HYMAP2_routing_struc(n)%imis
       endif   
       if(HYMAP2_routing_struc(n)%flowtype==4)then
        ...
        call HYMAP2_gen_urban_drain_maps(HYMAP2_routing_struc(n)%nseqall,&
                  ...
                  HYMAP2_routing_struc(n)%inletarea)  
       endif
#if (defined SPMD)
