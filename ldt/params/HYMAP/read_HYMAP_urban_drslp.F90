...
!
! !INTERFACE:
subroutine read_HYMAP_urban_drslp(n, array)
! !USES:
  use ESMF
...
! !ARGUMENTS: 
  ...
  inquire(file=trim(HYMAP_struc(n)%urban_drslpfile), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP urban_drslpfile, ',&
           trim(HYMAP_struc(n)%urban_drslpfile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%urban_drslpfile), access='direct',&
...
end subroutine read_HYMAP_urban_drslp
