...
!
! !INTERFACE:
subroutine read_HYMAP_urban_drman(n, array)
! !USES:
  use ESMF
...
! !ARGUMENTS: 
  ...
  inquire(file=trim(HYMAP_struc(n)%urban_drmanfile), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP urban_drmanfile, ',&
           trim(HYMAP_struc(n)%urban_drmanfile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%urban_drmanfile), access='direct',&
...
end subroutine read_HYMAP_urban_drman
