...
!
! !INTERFACE:
subroutine read_HYMAP_urban_drainage_outlet(n, array)
! !USES:
  use ESMF
...
! !ARGUMENTS: 
  ...
  inquire(file=trim(HYMAP_struc(n)%urban_drainage_outletfile), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP urban_drainage_outlet, ',&
           trim(HYMAP_struc(n)%urban_drainage_outletfile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%urban_drainage_outletfile), access='direct',&
...
end subroutine read_HYMAP_urban_drainage_outlet
