...
!
! !INTERFACE:
subroutine read_HYMAP_urban_inletarea(n, array)
! !USES:
  use ESMF
...
! !ARGUMENTS: 
  ...
  inquire(file=trim(HYMAP_struc(n)%urban_inletareafile), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP urban_inletareafile, ',&
           trim(HYMAP_struc(n)%urban_inletareafile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%urban_inletareafile), access='direct',&
...
end subroutine read_HYMAP_urban_inletarea
