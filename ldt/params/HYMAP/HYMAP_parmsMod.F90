...
!EOP
   ...
   do n=1,LDT_rc%nnest
      ...
      !pipe parameters:pipe_diameter pipelength r2pipedph r2pipewth inlet_d ifpipe

      call set_param_attribs(HYMAP_struc(n)%hymap_urban_drainage_outlet,&
           "HYMAP_urban_drainage_outlet")
      call set_param_attribs(HYMAP_struc(n)%hymap_urban_drlgh,&
           "HYMAP_urban_drlgh")
      call set_param_attribs(HYMAP_struc(n)%hymap_urban_drman,&
           "HYMAP_urban_drman")
      call set_param_attribs(HYMAP_struc(n)%hymap_urban_drrad,&
           "HYMAP_urban_drrad")
      call set_param_attribs(HYMAP_struc(n)%hymap_urban_inletarea,&
           "HYMAP_urban_inletarea")
      call set_param_attribs(HYMAP_struc(n)%hymap_urban_drslp,&
           "HYMAP_urban_drslp")
...
!           HYMAP_struc(n)%hymap_ref_q%vlevels))       
      ...
      !pipe allocate:pipe_diameter pipelength r2pipedph r2pipewth inlet_d ifpipe
      allocate(HYMAP_struc(n)%hymap_urban_drainage_outlet%value(&
           LDT_rc%lnc(n),LDT_rc%lnr(n),&
           HYMAP_struc(n)%hymap_urban_drainage_outlet%vlevels))
      allocate(HYMAP_struc(n)%hymap_urban_drlgh%value(&
           LDT_rc%lnc(n),LDT_rc%lnr(n),&    
           HYMAP_struc(n)%hymap_urban_drlgh%vlevels))
      allocate(HYMAP_struc(n)%hymap_urban_drman%value(&
           LDT_rc%lnc(n),LDT_rc%lnr(n),&
           HYMAP_struc(n)%hymap_urban_drman%vlevels))
      allocate(HYMAP_struc(n)%hymap_urban_drrad%value(&
           LDT_rc%lnc(n),LDT_rc%lnr(n),&
           HYMAP_struc(n)%hymap_urban_drrad%vlevels))
      allocate(HYMAP_struc(n)%hymap_urban_inletarea%value(&
           LDT_rc%lnc(n),LDT_rc%lnr(n),&
           HYMAP_struc(n)%hymap_urban_inletarea%vlevels))
      allocate(HYMAP_struc(n)%hymap_urban_drslp%value(&
           LDT_rc%lnc(n),LDT_rc%lnr(n),&
           HYMAP_struc(n)%hymap_urban_drslp%vlevels))
   ...
   enddo
   ! read HYMAP parameters:pipe_diameter pipelength r2pipedph r2pipewth inlet_d ifpipe
   call ESMF_ConfigFindLabel(LDT_config,"HYMAP urbandrainage outlet:",rc=rc)
   do n=1,LDT_rc%nnest
      call ESMF_ConfigGetAttribute(LDT_config,HYMAP_struc(n)%urban_drainage_outletfile,rc=rc)
      call LDT_verify(rc,'HYMAP urbandrainage outlet not specified')
   enddo
   call ESMF_ConfigFindLabel(LDT_config,"HYMAP urban_drlghfile:",rc=rc)
   do n=1,LDT_rc%nnest
      call ESMF_ConfigGetAttribute(LDT_config,HYMAP_struc(n)%urban_drlghfile,rc=rc)
      call LDT_verify(rc,'HYMAP urban_drlghfile: not specified')
   enddo
   call ESMF_ConfigFindLabel(LDT_config,"HYMAP urban_drmanfile:",rc=rc)
   do n=1,LDT_rc%nnest
      call ESMF_ConfigGetAttribute(LDT_config,HYMAP_struc(n)%urban_drmanfile,rc=rc)
      call LDT_verify(rc,'HYMAP urban_drmanfile: not specified')
   enddo
   call ESMF_ConfigFindLabel(LDT_config,"HYMAP urban_drradfile:",rc=rc)
   do n=1,LDT_rc%nnest
      call ESMF_ConfigGetAttribute(LDT_config,HYMAP_struc(n)%urban_drradfile,rc=rc)
      call LDT_verify(rc,'HYMAP urban_drradfile: not specified')
   enddo
   call ESMF_ConfigFindLabel(LDT_config,"HYMAP urban_inletareafile:",rc=rc)
   do n=1,LDT_rc%nnest
      call ESMF_ConfigGetAttribute(LDT_config,HYMAP_struc(n)%urban_inletareafile,rc=rc)
      call LDT_verify(rc,'HYMAP urban_inletareafile: not specified')
   enddo
   call ESMF_ConfigFindLabel(LDT_config,"HYMAP urban_drslpfile:",rc=rc)
   do n=1,LDT_rc%nnest
      call ESMF_ConfigGetAttribute(LDT_config,HYMAP_struc(n)%urban_drslpfile,rc=rc)
      call LDT_verify(rc,'HYMAP urban_drslpfile: not specified')
   enddo
   ...
   call LDT_readDomainConfigSpecs("HYMAP params",hymap_proj,&
    ...
    do n=1,LDT_rc%nnest
       ...
       !write pipe:pipe_diameter pipelength r2pipedph r2pipewth inlet_d ifpipe
       call LDT_gridOptChecks( n, "HYMAP urban_drainage_outletfile", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) 'reading '//trim(HYMAP_struc(n)%urban_drainage_outletfile)
       call read_HYMAP_urban_drainage_outlet(&
            n,HYMAP_struc(n)%hymap_urban_drainage_outlet%value(:,:,1))
       write(LDT_logunit,*) 'Done reading '//trim(HYMAP_struc(n)%urban_drainage_outletfile)

       call LDT_gridOptChecks( n, "HYMAP urban_drlghfile", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) 'reading '//trim(HYMAP_struc(n)%urban_drlghfile)
       call read_HYMAP_urban_drlgh(&
            n,HYMAP_struc(n)%hymap_urban_drlgh%value(:,:,1))
       write(LDT_logunit,*) 'Done reading '//trim(HYMAP_struc(n)%urban_drlghfile)

       call LDT_gridOptChecks( n, "HYMAP urban_drmanfile", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) 'reading '//trim(HYMAP_struc(n)%urban_drmanfile)
       call read_HYMAP_urban_drman(&
            n,HYMAP_struc(n)%hymap_urban_drman%value(:,:,1))
       write(LDT_logunit,*) 'Done reading '//trim(HYMAP_struc(n)%urban_drmanfile)

       call LDT_gridOptChecks( n, "HYMAP urban_drradfile", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) 'reading '//trim(HYMAP_struc(n)%urban_drradfile)
       call read_HYMAP_urban_drrad(&
            n,HYMAP_struc(n)%hymap_urban_drrad%value(:,:,1))
       write(LDT_logunit,*) 'Done reading '//trim(HYMAP_struc(n)%urban_drradfile)

       call LDT_gridOptChecks( n, "HYMAP urban_inletareafile", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) 'reading '//trim(HYMAP_struc(n)%urban_inletareafile)
       call read_HYMAP_urban_inletarea(&
            n,HYMAP_struc(n)%hymap_urban_inletarea%value(:,:,1))
       write(LDT_logunit,*) 'Done reading '//trim(HYMAP_struc(n)%urban_inletareafile)

       call LDT_gridOptChecks( n, "HYMAP urban_drslpfile", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) 'reading '//trim(HYMAP_struc(n)%urban_drslpfile)
       call read_HYMAP_urban_drslp(&
            n,HYMAP_struc(n)%hymap_urban_drslp%value(:,:,1))
       write(LDT_logunit,*) 'Done reading '//trim(HYMAP_struc(n)%urban_drslpfile)
...
!pipe:pipe_diameter pipelength r2pipedph r2pipewth inlet_d ifpipe
    call LDT_writeNETCDFdataHeader(n,ftn,dimID,&
         HYMAP_struc(n)%hymap_urban_drainage_outlet)
    call LDT_writeNETCDFdataHeader(n,ftn,dimID,&
         HYMAP_struc(n)%hymap_urban_drlgh)
    call LDT_writeNETCDFdataHeader(n,ftn,dimID,&
         HYMAP_struc(n)%hymap_urban_drman)
    call LDT_writeNETCDFdataHeader(n,ftn,dimID,&
         HYMAP_struc(n)%hymap_urban_drrad)
    call LDT_writeNETCDFdataHeader(n,ftn,dimID,&
         HYMAP_struc(n)%hymap_urban_inletarea)
    call LDT_writeNETCDFdataHeader(n,ftn,dimID,&
         HYMAP_struc(n)%hymap_urban_drslp)
  ...
  subroutine HYMAPparms_writeData(n,ftn)
    ...
    !pipe :pipe_diameter pipelength r2pipedph r2pipewth inlet_d ifpipe
    call LDT_writeNETCDFdata(n,ftn,HYMAP_struc(n)%hymap_urban_drainage_outlet)
    call LDT_writeNETCDFdata(n,ftn,HYMAP_struc(n)%hymap_urban_drlgh)
    call LDT_writeNETCDFdata(n,ftn,HYMAP_struc(n)%hymap_urban_drman)
    call LDT_writeNETCDFdata(n,ftn,HYMAP_struc(n)%hymap_urban_drrad)
    call LDT_writeNETCDFdata(n,ftn,HYMAP_struc(n)%hymap_urban_inletarea)
    call LDT_writeNETCDFdata(n,ftn,HYMAP_struc(n)%hymap_urban_drslp)
